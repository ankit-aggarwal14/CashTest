...
function AddUserSendEmailClicked(obj) {
    ...
    if ($("#addUserSendEmail")[0].checked === true) {        
        $("addUserPasswordBase").removeClass("d-none").addClass("d-none");
        $("#addUserConfirmPasswordBase").removeClass("d-none").addClass("d-none");
    }
    else {
        $("#addUserPasswordBase").removeClass("d-none");
        $("#addUserConfirmPasswordBase").removeClass("d-none");       
    }
...
function ValidateInput() {
    ...
    if ($("#addUserSendEmail")[0].checked !== true) {
        ...
        if (!flag)
            $("#spanPassword")[0].innerHTML = "The Password must be between 8 to 20 characters long and must contain atleast one upper case, one lower case, one numeric and one special character(!@.,#$%^&*)";
...
function OnUserDeleteClicked(selectedUserId) {
    DisplayConfirmationDialog({ //Confirming from the user, the delete operation and passing the required data
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.indexOf("success") > -1) {
                //On success, also removing the selected row from the Users table
                DisplayDialog({ Success: true, Message: response });
                $("#tblUsers tr[id='" + selectedUserId + "']").remove();
            }
            else {
                DisplayDialog({ Success: false, Message: response });
            }
...
function OnRoleDeleteClicked(selectedrow) {
    var row = $(selectedrow).parent().parent();
    console.log(row);

    var selectedRoleId = row;
    console.log(selectedRoleId);
    //var selectedRoleId = selectedrow;
...
function TriggerDeleteRole(selectedRoleId) {
    ...
    $.ajax({
        ...
        data: JSON.stringify(selectedRoleId.attr('id')),
        contentType: 'application/json; charset=utf-8',
        success: function (response) {
            ...
            if (response.indexOf("success") > -1) {
                ...
                $("#tableRoles tr[id='" + selectedRoleId.RoleId + "']").remove();
            }
            else {
                DisplayDialog({ Success: false, Message: response });
                preloader.remove();
...
function TriggerDeleteMultipleRole(selectedRoleId) {
    preloader.load();
    $.ajax({
        type: "POST",
        url: "/Roles/DeleteMultipleRoles",
        data: JSON.stringify(selectedRoleId.RoleId),
        contentType: 'application/json; charset=utf-8',
        success: function (response) {
            //Notifies the user on success and failure
            if (response.indexOf("success") > -1) {
                ...
                GetRoleList();
                DisplayDialog({ Success: true, Message: response });
                $('#delete_role').addClass('none');
                
            }
            else {
                DisplayDialog({ Success: false, Message: response });
                $('#tableRoles').find('tr').removeClass("highlighted");
                $('#delete_role').addClass('none'); preloader.remove();
            }
        },
        failure: function (response) {
            console.log(response.Message); preloader.remove();
        },
        error: function (response) {
            console.log(response.Message); preloader.remove();
        },
